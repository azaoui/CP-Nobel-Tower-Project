...
//import org.exoplatform.social.webui.profile.BuildingInfoUsersSearch;
...
import org.nobeltower.social.webui.profile.UIProfileUserSearch;
 ...
 */
@ComponentConfig(
  template = "classpath:groovy/social/webui/connections/CustomUIAllPeople.gtmpl",
  events = {
    @EventConfig(listeners = CustomUIAllPeople.ConnectActionListener.class),
    @EventConfig(listeners = CustomUIAllPeople.ConfirmActionListener.class),
    @EventConfig(listeners = CustomUIAllPeople.IgnoreActionListener.class),
    @EventConfig(listeners = CustomUIAllPeople.SearchActionListener.class),
    @EventConfig(listeners = CustomUIAllPeople.LoadMorePeopleActionListener.class)
  }
)
public class CustomUIAllPeople extends UIContainer {
  
  private static final Log LOG = ExoLogger.getLogger(CustomUIAllPeople.class);
   ...
   * @throws Exception
   */
  public CustomUIAllPeople() throws Exception {
  ...
  private List<Identity> loadPeople(int index, int length) throws Exception {
    ...
    ListAccess<Identity> listAccess = Utils.getIdentityManager().getIdentitiesByProfileFilter(lastOwner.getProviderId(), filter,
                                                                                              false);
    List<Identity> searchbybuildingsection = null;

    UIFormStringInput uibuilding= uiProfileUserSearch.getChildById("building");
    String buildingsearch= uibuilding.getValue();
    
    UIFormStringInput uifloor= uiProfileUserSearch.getChildById("floor");
    String floorcondition= uifloor.getValue();
    
    UIFormStringInput uiroom= uiProfileUserSearch.getChildById("room");
    String roomcondition= uiroom.getValue();
    
    Identity[] identities = listAccess.load(index, length);

    if(!buildingsearch.equals("building")&&buildingsearch!=null)
    {
    BuildingInfoUsersSearch bli = new BuildingInfoUsersSearch();
    //List<Identity> ecxluded = new ArrayList<Identity>();
     searchbybuildingsection =bli.search(buildingsearch,Arrays.asList(identities),0,30,"void-buildingName","asc");
    }
    
    //need to add the result of search by building to the list of identies already loaded by filter
    ...
    // Arrays.asList(identities).addAll(searchbybuildingsection);
   ...
   * @since Aug 18, 2011
   */
  static public class LoadMorePeopleActionListener extends EventListener<CustomUIAllPeople> {
    public void execute(Event<CustomUIAllPeople> event) throws Exception {
      CustomUIAllPeople uiAllPeople = event.getSource();
   ...
   */
  public static class ConnectActionListener extends EventListener<CustomUIAllPeople> {
    public void execute(Event<CustomUIAllPeople> event) throws Exception {
      CustomUIAllPeople uiAllPeople = event.getSource();
   ...
   */
  public static class ConfirmActionListener extends EventListener<CustomUIAllPeople> {
    public void execute(Event<CustomUIAllPeople> event) throws Exception {
      CustomUIAllPeople uiAllPeople = event.getSource();
      String userId = event.getRequestContext().getRequestParameter(OBJECTID);
      Identity invitedIdentity = Utils.getIdentityManager().getIdentity(userId, true);
   ...
   */
  public static class IgnoreActionListener extends EventListener<CustomUIAllPeople> {
    public void execute(Event<CustomUIAllPeople> event) throws Exception {
      CustomUIAllPeople   uiAllPeople = event.getSource();
      String userId = event.getRequestContext().getRequestParameter(OBJECTID);
   ...
   * @since Aug 25, 2011
   */
  static public class SearchActionListener extends EventListener<CustomUIAllPeople> {
    @Override
    public void execute(Event<CustomUIAllPeople> event) throws Exception {
